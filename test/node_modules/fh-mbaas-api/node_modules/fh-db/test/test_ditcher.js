          ...
          targetDb.addUser(own_app_config.database.auth.user, own_app_config.database.auth.pass, function(err, result){
            ...
            var ownAppDitch = new ditchhandler.Ditcher(own_app_config, logger, "9.8.7-Test Version", function() {
              ...
              ownAppDitch.doCreate(testData, function(err, res) {
                ...
                targetDb.collectionNames({namesOnly:true}, function(err, collNames){
                  logger.error(err);
                  assert.isNull(err);
                  logger.info(collNames);


                  collNames = collNames.filter(function(collName){
                    return collName == databaseName + "." + createDataOwnApp.type;
                  });
                  assert.equal(collNames.length, 1);
                  assert.equal(collNames[0], databaseName + "." + createDataOwnApp.type);
    ...
    db.open(function(err, targetDb){
      ...
      targetDb.authenticate(config.database.adminauth.user, config.database.adminauth.pass,{authSource:"admin"}, function(err, result){
        ...
        targetDb.collectionNames({namesOnly:true}, function(err, collNames){
          ...
          var expectedCollectionName = config.database.name + "." + "fh_" + createDataDitchApp.__fhdb + "_" + createDataDitchApp.type;

          collNames = collNames.filter(function(collName){
            return collName == expectedCollectionName;
          });
          assert.equal(collNames.length, 1);
          assert.equal(collNames[0], expectedCollectionName);
