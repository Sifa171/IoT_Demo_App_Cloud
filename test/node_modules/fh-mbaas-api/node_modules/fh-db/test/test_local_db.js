...
var testLocalMongoInstance = function (cb) {
  ...
  var newDb = new Db("FH_LOCAL", new Server("localhost", 27017), {fsync: true});

  newDb.open(function (err, targetDb) {
    assert.ok(!err);
    ...
    targetDb.dropDatabase(function (err, result) {
      ...
      local_db_no_mongo_string(createRequestLocal, function (err, result) {
        ...
        targetDb.open(function (err, targetDb) {
          ...
          targetDb.collectionNames({namesOnly: true}, function (err, collectionNames) {
            assert.ok(!err);

            collectionNames = collectionNames.filter(function (collName) {
              return (collName.indexOf("system.") == -1);
            });

            //Should only have 1 collection
            assert.equal(collectionNames.length, 1);
            //Now to get the collection to get the data
            assert.equal(collectionNames[0], "FH_LOCAL.fh_someAppThing_LocalType");
            targetDb.close();
...
var testSingleMongoInstance = function (cb) {
  ...
  var newDb = new Db("someRemoteApp", new Server("localhost", 27017), {fsync: true});
  ...
  newDb.open(function (err, targetDb) {
    ...
    targetDb.dropDatabase(function (err, result) {
      ...
      targetDb.addUser("user1", "pass1", function (err, result) {
        assert.ok(!err);
        targetDb.close();
        ...
        local_db_single_mongo_string(createRequestRemote, function (err, result) {
          ...
          targetDb.open(function (err, targetDb) {
            ...
            targetDb.collectionNames({namesOnly: true}, function (err, collectionNames) {
              ...
              assert.equal(collectionNames.length, 1);
              //Now to get the collection to get the data
              assert.equal(collectionNames[0], "someRemoteApp.RemoteType");
              targetDb.close();
...
var testReplicaMongoInstance = function (cb) {
  ...
  var newDb = new Db("someReplicaApp", new Server("localhost", 27017), {fsync: true});
  ...
  newDb.open(function (err, targetDb) {
    ...
    targetDb.dropDatabase(function (err, result) {
      ...
      targetDb.addUser("user2", "pass2", function (err, result) {
        assert.ok(!err);
        targetDb.close();
        ...
        local_db_replica_mongo_string(createRequestReplica, function (err, result) {
          ...
          targetDb.open(function (err, targetDb) {
            ...
            targetDb.collectionNames({namesOnly: true}, function (err, collectionNames) {
              ...
              assert.equal(collectionNames.length, 1);
              //Now to get the collection to get the data
              assert.equal(collectionNames[0], "someReplicaApp.ReplicaType");
              targetDb.close();
...
exports['test local and remote'] = function () {
  ...
  async.series([testLocalMongoInstance,
    ...
    testparseMongoConnectionString], function (err) {
    assert.ok(!err);
  });
